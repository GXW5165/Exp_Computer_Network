...
static void *tcp_cwnd_thread(void *arg)
{
	...
	while (tsk->state == TCP_ESTABLISHED && time_us < 1000000) {
		...
		fprintf(fp, "%d %u %u %u %u %d\n",
			time_us, tsk->cwnd, tsk->ssthresh,
			tsk->adv_wnd, tsk->snd_wnd, tsk->c_state);
		fflush(fp);
	}
...
int tcp_sock_wait_all_acked(struct tcp_sock *tsk)
{
	while (1) {
		...
		empty = list_empty(&tsk->send_buf);
		pthread_mutex_unlock(&tsk->send_buf_lock);

		pthread_mutex_lock(&tsk->sk_lock);
		done = empty && (tsk->snd_una == tsk->snd_nxt);
		alive = (tsk->state == TCP_ESTABLISHED || tsk->state == TCP_CLOSE_WAIT);
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	if (tsk->state == TCP_ESTABLISHED && !tsk->cwnd_record_on) {
		pthread_t cwnd_record;
		tsk->cwnd_record_on = 1;
		pthread_create(&cwnd_record, NULL, tcp_cwnd_thread, (void *)tsk);
		pthread_detach(cwnd_record);
	}
